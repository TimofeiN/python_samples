"""
Task desk is a class-structure

Have regular_expression_parse queues:
1) Base, where tasks are taken from, solved and sent to the list of solved
2) Queue for revision, when unresolved tasks from the first queue are sent
to correct the decision
3) The list of solved tasks, where tasks are moved from the base queue, or
from waiting for revision list.
"""
...
class TaskDesk:
    ...
    def show_desk(self):
        print(f'main    - {self.main_tasks}\n'
              f'working - {self.working_tasks}\n'
              f'decided - {self.decided_tasks}\n')
